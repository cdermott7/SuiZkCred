...
export default function ProofModal({ 
  ...
  const [uploadedDocument, setUploadedDocument] = useState<{ file: File; preview: string; documentType: string } | null>(null);
  ...
  // Generate a simplified proof for the demo
  async function generateSimplifiedProof() {
    ...
    try {
      // Use the utility function to generate the proof
      console.log('Creating demo proof...');
      
      // Add a user identifier to the credential nullifier
      const proofResult = await generateCredentialProof(
        credentialType,
        expiryDays
      );
      
      // Add userId to the metadata
      proofResult.metadata.userId = userId;
      
      console.log('Demo proof created:', proofResult);
      return proofResult;
    } catch (error: any) {
  ...
  // Step 2: Store off-chain metadata
  ...
  // Step 4: Mint credential on-chain
  async function mintCredential(proof: any, metadataKey: any) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Step 1: Generate simplified proof (for demo)
      const proofResult = await generateSimplifiedProof();
      if (!proofResult) {
        setStep(1);
        return;
      }
      ...
      const storageKey = await storeMetadata(proofResult);
      
      // Step 3: Register nullifier in Supabase
      try {
        await registerNullifier(proofResult);
      } catch (error) {
      ...
      // Step 4: Mint credential on-chain
      await mintCredential(proofResult, storageKey);
      
      // Create the credential object
      const timestamp = Date.now();
      const expirationTimestamp = timestamp + (expiryDays * 24 * 60 * 60 * 1000);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-full max-w-xl p-0 bg-white rounded-xl shadow-xl overflow-hidden">
        {/* Header */}
              ...
              <div className="mb-6">
                ...
                {showUploader && (
                  <div className="mb-6 bg-gray-50 p-4 rounded-md border border-gray-200" data-demo-target="document-upload">
                    <DocumentUploader
                      onUploadComplete={(file, preview, documentType) => {
                        setUploadedDocument({ file, preview, documentType });
                        // Auto-set credential type based on document type
                        if (documentType === "Passport") {
                          setCredentialType(CredentialType.ADVANCED_KYC);
                        } else if (documentType === "Driver's License") {
                          setCredentialType(CredentialType.BASIC_KYC);
                        }
                      }}
